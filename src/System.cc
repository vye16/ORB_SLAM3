...
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const cv::Mat &mask, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    if(settings_ && settings_->needToResize()){
        ...
        {
          cv::Mat resizedMask;
          cv::resize(mask, resizedMask, settings_->newImSize());
          maskToFeed = resizedMask;
        }
    }
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular(imToFeed, maskToFeed, timestamp,filename);
...
void System::SaveTrajectoryEuRoC(const string &filename)
{
        ...
        lend=mpTracker->mlRelativeFramePoses.end(); lit!=lend; lit++, lRit++, lT++, lbL++)
    {
        ...
        f << setprecision(6) << 1e9*(*lT) << " " <<  setprecision(9) << t(0) << " " << t(1) << " " << t(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
...
void System::SaveKeyFrameTrajectoryEuRoC(const string &filename)
{
    ...
    std::cout << vpKFs.size() << " keyframes" << std::endl;

    ofstream f;
    f.open(filename.c_str());
    f << fixed;

    Sophus::SE3f Twc;  // world to camera
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        KeyFrame* pKF = vpKFs[i];
        std::cout << "Processing KF " << i << std::endl;
        ...
        f << " " <<  setprecision(9) << t(0) << " " << t(1) << " " << t(2);
        f << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
    }
    cout << "Done writing keyframes." << endl;
    f.close();
